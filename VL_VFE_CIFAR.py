...
parser.add_argument('--intermediate_dim', type=int, default=64, help='dimension of encoded vector')
...
class Net(nn.Module):
    ...
    def get_mask_inference(self, channel_noise, threshold = args.thr_ratio):
        mu = self.gamma_mu(channel_noise)
...
def inference_accuracy(output, labels):
...
def train(model=model):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch)%10 == 0:
            data_loader = torch.utils.data.DataLoader(trainset,batch_size=args.batch, shuffle=True,
        ...
        for i, (x, y) in enumerate(data_loader):
            ...
            if i == 0:
                ...
                print('KL:{:.4f}'.format(KL.item()),'max magnitude:{:.4f}'.format(max_mag.item()),'l2 norm:{:.4f}'.format(total_norm.item()))
            '''
        ...
        scheduler.step()
        #accuracy_result = inference_accuracy(output,y)
        ...
        if epoch > 180:
            if (acc > test_acc and pruned_number == pruned_dim) or pruned_number > pruned_dim:
                ...
                print('best ckpt:',test_acc,'number:',args.intermediate_dim,'pruned_number',pruned_dim,'beta:',args.penalty,'threshold:',args.thr_ratio)
                torch.save({'model': saved_model1}, './model_cifar/VL_VFE_dynamic_best_ckpt_hid:{}_penalty:{}_model1.pth'.format(args.intermediate_dim,args.penalty))
    print()
    print('best acc:',test_acc,'number:',args.intermediate_dim,'pruned_number',pruned_dim,'beta:',args.penalty,'threshold:',args.thr_ratio)
    torch.save({'model': saved_model1}, './model_cifar/VL_VFE_dynamic_hid:{}_remain:{}_penalty:{}_acc:{}_epoch:{}_model1.pth'.format(args.intermediate_dim,args.intermediate_dim - pruned_dim,args.penalty,test_acc,epoch))
...
if __name__=='__main__':
    ...
    if args.test == 0:
        ...
        t = 20
        for i in range (t):
            acc, _ = test(0,args.channel_noise)
            avg += acc
        print('Noise:',args.channel_noise,'Accuracy:',avg/t)
